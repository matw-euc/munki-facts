'''Returns a fact to indicate if this machine can be upgraded to macOS 13 sonoma'''
...
#   Install macOS Sonoma.app/Contents/SharedSupport/SharedSupport.dmg - mount this
#       /Volumes/Shared Support/com_apple_MobileAsset_MacSoftwareUpdate/d2d63d5f9a352d23b18f16952753191cd63111b0.zip 
...
#                    - 'SupportedProductTypes' from d2d63d5f9a352d23b18f16952753191cd63111b0/AssetData/boot/Restore.plist
#                    - 'SupportedModelProperties' from d2d63d5f9a352d23b18f16952753191cd63111b0/AssetData/boot/PlatformSupport.plist
...
def get_minor_system_version():
    '''Returns 19 for Sonoma, 18 for Ventura, 17 for Monterey, etc.'''
    darwin_version = int(os.uname()[2].split('.')[0])
    return darwin_version - 4


def is_supported_system_version():
    '''Returns True if current macOS version is 10.12 through 13.x,
    False otherwise'''
    macos_minor_version = get_minor_system_version()
    if macos_minor_version >= 19:
        return False
...
def fact():
    '''Return our sonoma_upgrade_supported fact'''
    if is_virtual_machine():
        return {'sonoma_upgrade_supported': True}
    if ((is_supported_model()) and
            is_supported_system_version()):
        return {'sonoma_upgrade_supported': True}
    return {'sonoma_upgrade_supported': False}
